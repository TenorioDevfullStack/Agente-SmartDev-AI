...
async function req(path,body,method=body?'POST':'GET'){const r=await fetch(f.url+'/api'+path,{method,headers:{'content-type':'application/json','x-painel-request':'1',cookie},...(body?{body:JSON.stringify(body)}:{})});if(r.headers.get('set-cookie'))cookie=r.headers.get('set-cookie').split(';')[0];return {status:r.status,data:await r.json()};}
try{
 ...
 await f.db.collection('conversas').insertOne({_id:'5511999990001',pausado:true});
 await f.db.collection('mensagens').insertOne({numero:'5511999990001',content:'teste'});
 await f.db.collection('leads').insertOne({_id:'5511999990001',nome:'Teste'});
 await f.db.collection('recados').insertOne({numero:'5511999990001',assunto:'Teste'});
 let apagado=await req('/prospeccao/contatos/5511999990001',undefined,'DELETE');assert.equal(apagado.status,200);
 assert.equal(await f.db.collection('prospeccao_contatos').countDocuments({_id:'5511999990001'}),0);
 assert.equal(await f.db.collection('conversas').countDocuments({_id:'5511999990001'}),0);
 assert.equal(await f.db.collection('mensagens').countDocuments({numero:'5511999990001'}),0);
 assert.equal(await f.db.collection('leads').countDocuments({_id:'5511999990001'}),0);
 assert.equal(await f.db.collection('recados').countDocuments({numero:'5511999990001'}),0);
 assert.equal((await req(url,{empresa:'Clínica [A]',telefone:'11999990001'})).status,201);
 ...
 assert.equal(f.enviados.length,0);console.log('PASS: autenticação, criação sem planilha, duplicidade, busca literal, filtros, exclusão limpa, recadastro, histórico e paginação; nenhum envio.');
